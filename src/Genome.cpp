...
#error "No objective chosen"
	...
	void Genome::move() {
		...
		while (!r)
		{ // Keep trying until we find a valid read
	...
	{
		for (iteration_t i = 0; i < this->fAccept.LENGTH; ++i)
			this->fAccept.buffer[i] = 0;
		for (iteration_t i = 0; i < this->pBad.LENGTH; ++i)
			this->pBad.buffer[i] = 0;
	...
	void Genome::optimize(bool debug)
	{
		...
		while (range.start + WINDOW_SIZE <= numberOfSites)
		{
			...
			if (cpuSeconds > tmp + 50)
			{ // For Debugging to break out if program can't find optimal solution
				break;
			}
		}
	...
	void Genome::autoSchedule(iteration_t iterations)
	{
		...
		double tInitial = 1;
		while (this->findPbad(tInitial) < TARGET_PBAD_START)
			tInitial *= 2;
		while (this->findPbad(tInitial) > TARGET_PBAD_START)
			tInitial /= 2;
		while (this->findPbad(tInitial) < TARGET_PBAD_START)
		...
		while (this->findPbad(tEnd) > TARGET_PBAD_END)
			tEnd /= 2;
		while (this->findPbad(tEnd) < TARGET_PBAD_END)
			tEnd *= 1.2;
		...
		if (curIteration % (REPORT_INTERVAL / 2) == 0)
		{
			...
			if (fracTime() - prev_retreat_frac > 2 * SMALL_RETREAT &&
				(((fracTime() > 0.3 || pBad < 0.2) && factor > 16) || // 14 to 22 seems to work well
				 ((fracTime() > 0.5 || pBad < 0.1) && factor > 8)))
			{ // quarter to half the above works well?
			...
			if (retreat > 0.0)
			{
				...
				// REMOVED COUT HERE
				// cout << ", too big by a factor of " << factor << "(" << TARGET_MEC << ", " << windowMeanCoverage() << ")" << endl;
